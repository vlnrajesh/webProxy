...
def proxy_fork(conn,client_addr):
    try:
        ...
        file_object = _client_socket.makefile('r',0)
        file_object.write("GET"+"http://"+dest_url+" HTTP/1.0\n\n")
        buff = file_object.readlines()
        logging.debug("Connected to %s at %s" %(dest_url,dest_port))
        temp_file = open("./" + dest_url, "wb")
